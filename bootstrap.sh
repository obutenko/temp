...
# Install salt
sudo echo "deb [arch=amd64] http://mirror.mirantis.com/testing/saltstack-2016.3/xenial xenial main" > /etc/apt/sources.list.d/saltstack.list
...
apt-get install python-apt-common=1.1.0~beta1build1 -y --allow-downgrades
apt-get install python3-distupgrade=1:16.04.25 -y --allow-downgrades
WORKDIR=${WORKDIR:-'/tmp/'}
cd ${WORKDIR}
